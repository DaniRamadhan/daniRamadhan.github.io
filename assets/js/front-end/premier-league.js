import {premierLeague,ENDPOINT_PREMIER_LEAGUE,topScorePl,ENDPOINT_TOPSCORE_PL} from"../back-end/api-premierLeague.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    premierLeague.then(dataJson => {
        getStanding(dataJson);
    }).catch(err => {
        console.log(err)
    })

    topScorePl.then( APITopScore => {
...
function getStanding(dataJson){
    if("caches" in window) {
        caches.match(ENDPOINT_PREMIER_LEAGUE).then(response => {
            if(response) {
...
function getTeam(dataJson){
    ...
    dataJson.standings[0].table.forEach(elements => { 
     ...
     <div class="card-image waves-effect waves-block waves-light">
       <img class="activator responsive-img" src="${elements.team.crestUrl.replace(/^http:\/\//i, 'https://')}" >
     </div>
...
function showTopScore(APITopScore){
    ...
    if ("caches" in window) {
        caches.match(ENDPOINT_TOPSCORE_PL).then(response => {
            if(response) {
                response.json().then(dataCache => { 
