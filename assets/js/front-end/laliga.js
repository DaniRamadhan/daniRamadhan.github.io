import {laliga , ENDPOINT_laliga,ENDPOINT_TOPSCORE_LL,topScoreLL} from"../back-end/api-laliga.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    laliga.then(dataJson => {
        getStanding(dataJson);
    }).catch(err => {
        console.log(err)
    })
    topScoreLL.then( APITopScore => {
...
function getStanding(dataJson){
    if("caches" in window) {
        caches.match(ENDPOINT_laliga).then(response => {
            if(response) {
...
function showTopScore(APITopScore){
        if ("caches" in window) {
            caches.match(ENDPOINT_TOPSCORE_LL).then(response => {
                if(response) {
                    response.json().then(dataCache => { 
