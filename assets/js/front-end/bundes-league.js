import {bundesLeague,ENDPOINT_BUNDES_LEAGUE, topScoreBL,ENDPOINT_TOPSCORE_BL} from"../back-end/api-bundesLeague.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    bundesLeague.then(dataJson => {
        getStanding(dataJson);
    }).catch(err => {
        console.log(err)
    })
    topScoreBL.then( APITopScore => {
...
function getStanding(dataJson){
    if("caches" in window) {
        caches.match(ENDPOINT_BUNDES_LEAGUE).then(response => {
            if(response) {
    ...
    function getTeam(dataJson){
     ...
     function showTopScore(APITopScore){
        if ("caches" in window) {
            caches.match(ENDPOINT_TOPSCORE_BL).then(response => {
                if(response) {
                    response.json().then(dataCache => { 
